...
> Track 4 — Agent UX
> Branch: `module/17-approvals`

## What you'll build

For destructive tools (Write/Edit/Bash) prompt `y/n` unless `--yolo`. Add an allowlist file.
...
Run `runner test --module 16` to confirm the prior module is green before starting (if applicable).
...
- [ ] Implement the required behavior to pass `runner test --module 17`.
- [ ] Articulate at least one trade-off you encountered while building it.

## What's already done for you

The starter branch (`module/17-approvals`) contains all earlier modules' solved code. The skeleton for this module's new behavior is in place with `# TODO` markers; your job is to fill them in.
...
3. Run `runner test --module 17` until green.
...
- **Name:** `n declines write`
- **Prompt / setup:** `Write 'x' to out.txt`
- **Assertions (high-level):** stdin sends 'n\n'; file_absent out.txt

See `test.yml` for the full set. Run locally:

```bash
runner test --module 17
```
