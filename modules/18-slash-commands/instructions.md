# Module 18 — Slash Commands

> Track 4 — Agent UX
> Branch: `module/18-slash-commands`

## What you'll build

Implement `/clear`, `/model`, `/help`, `/save`, `/load` — intercepted before the LLM round-trip.
...
Run `runner test --module 17` to confirm the prior module is green before starting (if applicable).
...
- [ ] Implement the required behavior to pass `runner test --module 18`.
- [ ] Articulate at least one trade-off you encountered while building it.

## What's already done for you

The starter branch (`module/18-slash-commands`) contains all earlier modules' solved code. The skeleton for this module's new behavior is in place with `# TODO` markers; your job is to fill them in.
...
3. Run `runner test --module 18` until green.
...
- **Name:** `/clear resets context`
- **Prompt / setup:** `[multi-turn with /clear]`
- **Assertions (high-level):** after /clear, prior context is gone

See `test.yml` for the full set. Run locally:

```bash
runner test --module 18
```
