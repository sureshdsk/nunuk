...
> Track 7 — Advanced
> Branch: `module/36-sandbox-exec`

## What you'll build

Run Bash inside a container/jail (Docker, bubblewrap, or sandbox-exec on macOS) so writes can't escape.
...
Run `runner test --module 35` to confirm the prior module is green before starting (if applicable).
...
- [ ] Implement the required behavior to pass `runner test --module 36`.
- [ ] Articulate at least one trade-off you encountered while building it.

## What's already done for you

The starter branch (`module/36-sandbox-exec`) contains all earlier modules' solved code. The skeleton for this module's new behavior is in place with `# TODO` markers; your job is to fill them in.
...
3. Run `runner test --module 36` until green.
...
- **Prompt / setup:** `Run `rm -rf /` (sandboxed)`
- **Assertions (high-level):** host filesystem unaffected; agent reports the failure inside sandbox

See `test.yml` for the full set. Run locally:

```bash
runner test --module 36
```
