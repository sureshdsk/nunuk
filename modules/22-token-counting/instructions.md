# Module 22 — Token Counting

> Track 5 — Robustness
> Branch: `module/22-token-counting`

## What you'll build

Wire `tiktoken` for OpenAI; add an Anthropic tokenizer shim. Display per-turn and cumulative tokens.
...
21-stuck-loop-detection

Run `runner test --module 21` to confirm the prior module is green before starting (if applicable).
...
- [ ] Implement the required behavior to pass `runner test --module 22`.
- [ ] Articulate at least one trade-off you encountered while building it.

## What's already done for you

The starter branch (`module/22-token-counting`) contains all earlier modules' solved code. The skeleton for this module's new behavior is in place with `# TODO` markers; your job is to fill them in.
...
3. Run `runner test --module 22` until green.
...
- **Name:** `token counter visible`
- **Prompt / setup:** `Hello`
- **Assertions (high-level):** stderr or status line contains 'tokens:' followed by a number

See `test.yml` for the full set. Run locally:

```bash
runner test --module 22
```
