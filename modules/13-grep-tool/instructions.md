...
> Track 3 — File Ops
> Branch: `module/13-grep-tool`

## What you'll build

Implement `Grep(pattern, path, glob, output_mode)` powered by ripgrep with a Python fallback.
...
Run `runner test --module 12` to confirm the prior module is green before starting (if applicable).
...
- [ ] Implement the required behavior to pass `runner test --module 13`.
- [ ] Articulate at least one trade-off you encountered while building it.

## What's already done for you

The starter branch (`module/13-grep-tool`) contains all earlier modules' solved code. The skeleton for this module's new behavior is in place with `# TODO` markers; your job is to fill them in.
...
3. Run `runner test --module 13` until green.
...
- **Name:** `grep TODO`
- **Prompt / setup:** `Find every TODO in the codebase`
- **Assertions (high-level):** tool_call_sequence contains Grep; stdout includes the TODO matches

See `test.yml` for the full set. Run locally:

```bash
runner test --module 13
```
