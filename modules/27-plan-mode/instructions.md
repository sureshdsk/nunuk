...
> Branch: `module/27-plan-mode`

## What you'll build

`--plan` flag forbids Write/Edit/Bash; agent must produce a markdown plan instead.
...
Run `runner test --module 26` to confirm the prior module is green before starting (if applicable).
...
- [ ] Implement the required behavior to pass `runner test --module 27`.
- [ ] Articulate at least one trade-off you encountered while building it.

## What's already done for you

The starter branch (`module/27-plan-mode`) contains all earlier modules' solved code. The skeleton for this module's new behavior is in place with `# TODO` markers; your job is to fill them in.
...
3. Run `runner test --module 27` until green.
...
- **Name:** `plan mode refuses Write`
- **Prompt / setup:** `Refactor app.py`
- **Assertions (high-level):** no Write tool calls; stdout contains markdown plan headings

See `test.yml` for the full set. Run locally:

```bash
runner test --module 27
```
